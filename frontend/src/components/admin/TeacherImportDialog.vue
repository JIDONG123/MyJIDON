<template>
  ...
  >
    ...
    <div v-if="step === 0" class="step-body">
      <div class="step-tip">
        <p>请先下载标准模板，按要求填写教师账号信息。</p>
        <p>初始密码默认为工号，教师首次登录必须修改密码。</p>
        <ul class="rule-list">
          <li>带 * 为必填：用户名、真实姓名、工号、邮箱、学院 / 部门</li>
          <li>电话号码为选填；负责班级请在「班级管理」中维护</li>
    ...
    <div v-else-if="step === 2" class="step-body">
      ...
      <div class="preview-table-wrap">
        <el-table :data="preview?.rows || []" border size="small" max-height="360">
          ...
          <el-table-column prop="teacherNo" label="工号" min-width="100" show-overflow-tooltip />
          <el-table-column prop="email" label="邮箱" min-width="150" show-overflow-tooltip />
          <el-table-column prop="department" label="学院 / 部门" min-width="120" show-overflow-tooltip />
    ...
    <template #footer>
      ...
      <el-button v-if="step === 2" plain @click="reupload">重新上传</el-button>
      <el-button
        v-if="step === 2"
        type="primary"
        :loading="confirming"
...
import {
  ...
  downloadTeacherImportResult,
} from '../../api/teacherImport'
...
const downloadTemplate = async () => {
  ...
  try {
    ...
    await triggerBlobDownload(blob, '教师账号导入模板.xlsx', '模板下载失败')
  } catch (e) {
...
const runPreview = async () => {
  ...
  try {
    const res = await previewTeacherImport(selectedFile.value)
    if (res.success) {
...
const runConfirm = async () => {
  ...
  try {
    const res = await confirmTeacherImport(preview.value.batchId)
    if (res.success) {
...
const downloadResult = async () => {
  ...
  try {
    const blob = await downloadTeacherImportResult(batchId)
    await triggerBlobDownload(blob, `teacher-import-result-${batchId}.xlsx`, '下载失败')
  } catch (e) {
