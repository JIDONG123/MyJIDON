<template>
  <el-dialog v-model="visible" title="教师导入记录" width="960px" destroy-on-close @open="loadBatches">
    ...
    <el-drawer v-model="detailVisible" title="导入批次详情" size="720px" destroy-on-close>
      <template v-if="detail">
        ...
        <el-table :data="detail.rows" border size="small" max-height="520">
          ...
          <el-table-column prop="teacher_no" label="工号" min-width="100" show-overflow-tooltip />
...
import {
  listTeacherImportBatches,
  getTeacherImportBatchDetail,
...
const loadBatches = async () => {
  loading.value = true
  try {
    const res = await listTeacherImportBatches({
      page: page.value,
...
const openDetail = async (row) => {
  ...
  detail.value = null
  try {
    const res = await getTeacherImportBatchDetail(row.id)
