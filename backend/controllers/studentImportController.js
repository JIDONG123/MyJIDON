...
async function downloadTemplate(_req, res) {
  try {
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename="student-import-template.xlsx"; filename*=UTF-8''${encodeURIComponent('学生账号导入模板.xlsx')}`
    );
    res.setHeader(
      'Content-Type',
...
async function importPreview(req, res) {
  try {
    ...
    const savedPath = await studentImportService.saveUploadFile(
      req.file.buffer,
      req.file.originalname
    );
    const data = await studentImportService.previewImport({
...
async function importConfirm(req, res) {
  try {
    ...
    const data = await studentImportService.confirmImport(batchId, req.user.id);
    res.json({ success: true, data });
  } catch (e) {
...
async function listImportBatches(req, res) {
  try {
    ...
    const result = await studentImportService.listBatches({ page, pageSize, status: status || undefined });
...
async function getImportBatchDetail(req, res) {
  try {
    const batchId = Number(req.params.batchId);
    const detail = await studentImportService.getBatchDetail(batchId);
    if (!detail) {
...
async function downloadImportResult(req, res) {
  try {
    const batchId = Number(req.params.batchId);
    const buf = await studentImportService.buildResultExcelBuffer(batchId);
    res.setHeader(
      'Content-Disposition',
      `attachment; filename="student-import-result-${batchId}.xlsx"`
    );
    res.setHeader(
      'Content-Type',
      'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
...
async function getSummary(_req, res) {
  try {
    const data = await studentImportService.getStudentSummary();
    res.json({ success: true, data });
  } catch (e) {
